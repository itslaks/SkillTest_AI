...
|-- seeds/        # Seed data, smoke tests, and test fixture generators
`-- fixes/        # One-off SQL patches
```

## Running Migrations

Open the Supabase SQL Editor and run the files in `migrations/` in numeric order (`001` through `050`).
...
| 049-050 | SaaS tenancy/billing/SSO foundation and proctoring validation program |
...
These SQL files are one-off operational patches. Do not re-run them on a production database unless you have checked the current schema and data state.
