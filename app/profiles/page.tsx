...
export default async function ProfilesPage() {
  const [{ data: profiles, error }, profile] = await Promise.all([
    getVisibleProfiles(),
    getUserProfile(),
  ])
  const dashboardHref = ['admin', 'manager', 'training_coordinator', 'trainer'].includes(profile?.role || '')
    ? '/manager'
    : '/employee'
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-4 md:p-8">
      <section className="rounded-3xl border border-zinc-900 bg-black p-6 text-white shadow-[0_30px_100px_rgba(0,0,0,0.35)] md:p-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          ...
          <Button asChild className="w-fit rounded-full bg-white text-black hover:bg-zinc-200">
