...
async function removeOrphanEmployeeAuthUsers(adminClient: ReturnType<typeof createAdminClient>, email: string) {
  try {
    const cleanup = await cleanupOrphanEmployeeAuthUsersByEmail(adminClient, email, { reason: 'before_self_signup' })
    if (cleanup.warnings.length > 0) {
      console.warn('[auth] orphan auth cleanup warnings before signup:', {
        email,
        warnings: cleanup.warnings,
      })
    }
    const conflicting = cleanup.keptAuthUsers.find((authUser) => {
      const role = String(authUser.user_metadata?.role || 'employee')
      return role !== 'employee'
    })
    if (conflicting) {
      const role = String(conflicting.user_metadata?.role || 'unknown')
